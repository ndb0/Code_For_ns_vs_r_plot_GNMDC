...
subroutine set_efolds(coff,nstar,info,x0,nch)  
	...
	implicit none
		...
		character(3) rff
		integer nch
        real(dl) detnorm,n,phi_end,co5p,co5n,co5star,nstar,hn,neta
        ...
        do while(n.ge.nef*3.d0)
         print *,"n>3nef: n,coff(5),x0,coff(15)",n,coff(5),x0,coff(15)
          co5p=coff(nch)
          coff(nch)=coff(nch)/1.10d0
          call run_back(coff,n,x0,phi_end,hn)
        enddo  
        ...
        if(detnorm.le.0.d0)then
         ...
         ! goto 908
          ...
          if(detnorm.le.0.d0)then
             ...
             co5n=coff(nch)/1.05d0
            co5p=coff(nch)
          endif   
      !    908 co5n=coff(5)
        else
        co5p=coff(nch)
    588 coff(nch)=coff(nch)/1.2d0
        call run_back(coff,n,x0,phi_end,hn)
        detnorm=n-nef
    ...
    !    neta=nch
            do while (abs(nstar-nef)/nef.ge.1e-3)
                ...
                call get_efolds(co5star,nstar,co5p,co5n,nch,info,x0,coff,hn) 
    ...
    !Ipara is private by default
        ...
        DO Ipara=1,iparacount
            if(efolds(ipara).lt.(nef*3.d0))then
              ercount1=ercount1+1
            if(phi_end(ipara).lt.(copara(ipara)*coff(1)*0.7))then
                ercount2=ercount2+1
            endif
            ...
            !print **,"cof5,n,phi_,cof5*x0 :", copara(ipara),efolds(ipara),phi_end(ipara),copara(ipara)*x0
        ENDDO
        ...
        if((ercount1.ge.1).and.(ercount2.ge.1))then
            ...
            co5p=copara(iparacount)*1.05d0
            co5n=copara(iparacount)
         else if((ercount1.lt.1).and.(ercount2.ge.1))then   
            co5p=copara(1)
            co5n=copara(1)/1.05d0
         else
            !print **,"error unrecoverable: need to change phi_ini"
            read(*,*) rff
         endif   
...
71		enddo
    ...
    subroutine run_back_assignvalues(coff,hn)
        use mod1
        use mod2
        use mod3
        use mod4
      ...
      znt(i)=znt1
        ...
        do while (ee.lt.1.0)
				...
				if (n.ge.(nef*3.d0)) then
				 ...
				 print *, "odd error in assignvals spectral 487"
				 stop
                end if  
71		enddo
        phi_end=phi00
