...
!	coff(6)=3.67e-11      ! the overall coefficient in potential i.e. m^2 here.
	...
	coff(16)=100.0
        ...
        coff(15)=3.296979904174805                                        !17.2907278468742379026076582001494    ! initial value of phi
...
344		error(25)=0
      ...
      !  OPEN(unit=1522,file="../Nidra",action='write',position='append')
       ...
       !        write(1522,*) "Nan-Negetive Err: in brat:",iss0,coff(24),brat,coff(16)
                ...
                coff(16)=coff(16)*0.9
        !        write(1522,*) "Nan-Negetive Err: in else:",iss0,coff(24),brat,coff(16)
        endif 
             iss0=iss0+1   
...
243     call get_N(coff,efold,info,x0,jin) !	call set_efolds(coff,efold,info,x0)
       ! print *," efold=",efold, "coff(15)=",coff(15),"coff(10)=",coff(10),"coff(16)=",coff(16),nef
...
1248	aloopi=0
	i=1
	dgap1=.4
258	sp00=2.10521e-9!2.20652e-9   !2.10521e-9
	...
	dk=.1
       ...
       !##################################################################
222	WRITE(filename,'(a,i4.4,a,i4.4,a)') '../data/reheating_d_check',jin,"ln",ljin,".TXT"
...
!#############################################################################################
  ...
  !  Write(*,*) "coffs=",(coff(i),i=1,16)
   859  call pwr(kk,power,power1,phi_exit,fsrp,j,coff,tpp,k_infl,info,x0,ljin,jin,k_ini)
...
!###########################################################################################################
   ...
   ! real(dl) sp,tp,sd,ns,nt,k,dk
	...
	do while (kpara1.le.k_infl)
		...
		if ((kpara1.ge.1e-2).and.(kpara1.le.0.1)) then	
	!		!dk=k*0.1
			kpara(ipara)=kpara1
	!		!print **,ipara, kpara(ipara)
			...
			!k=k*1.05
                ...
                else if ((kpara1.ge.5e-4).and.(kpara1.le.1e-2)) then
			!dk=k*0.4
            kpara(ipara)=kpara1
            !print **,ipara, kpara(ipara)
			ipara=ipara+1
...
!!	CALL OMP_SET_NUM_THREADS(48)
    ...
    DO Ipara=1,iparacount-1
      ...
      !  open (unit = 11, file = filename,action='write',position='append')
        ...
        tp11=tppw(ipara)
        r11=tp11/sp11
       write (*,*) k11,sp11,tp11,srsp11,r11
...
!	do 10 i = 1, 3
 ...
 ! 10  continue
	...
	!psdata(j,13)=N
	!psdata(j,18)=k
	call subhorizon_evolution(k_gs, phi0_gs,phi_n0_gs,H0_gs,N_gs,hn,coff,info,x0)
	!!print **, "check",j
	r_gs=[phi0_gs,phi_n0_gs,H0_gs]
	a_gs=a0*exp(N_gs)
  ...
  call perturbation_evolution(phi0_gs,phi_n0_gs,H0_gs,qr0_gs,qc0_gs,q_nr0_gs,q_nc0_gs,vr0_gs,vc0_gs,v_nr0_gs,v_nc0_gs, &
             N_gs,k_gs,hs,coff,srsp_gs,info,x0)
	r_gs=[phi0_gs,phi_n0_gs,H0_gs]
	a_gs=a0*exp(N_gs)
	!!print **,"=====###############======at N H",H0,N
