...
double precision, parameter:: nef=75.0d0,phi_ini=9.10d0,a00=2.3470437189152191E-003,PI=4.D0*DATAN(1.D0)
...
!potential function
		...
		!V=cof*0.5*phi**2
		V= cof*(1.d0-(1.d0*phi**4)/f**4)
                
                !cof*(1.d0-  &
                        !  1.d0/Exp(0.816496580927726d0*phi))**2
...
! G inflation term : arXiv:2001.05909
	...
	real(dl)   function  theta_diped(coff,phi)  

		implicit none
		real(dl)   phi
		real(dl) coff(25)
		
		theta_diped=theta_d(coff,phi)+theta_mono(coff,phi) 
...
! G inflation term : arXiv:2001.05909
	...
	real(dl)   function  theta_d(coff,phi)  
		...
		s2=coff(19)
                ...
                theta_d=-A0/(1.d0+(phi-1.d0*phic)**p/sigma)**(1.d0*q)
...
!    1.d0*phic)**p/sigma)**(1.d0*q)))+0.4*A0*phi**4/phic**4
	...
	real(dl)   function  thetaphi_mono(coff,phi)  
		...
		phic=coff(1)*coff(15)
                t=coff(25)
               thetaphi_mono=(A1*t*(phi)**(-1.d0+t))/coff(15)**t      
	...
	real(dl)   function  thetaphiphi_mono(coff,phi)  
		...
		thetaphiphi_mono=(A1*(-1.d0+t)*t*(phi)**(-2.d0+t))/coff(15)**t
       ! if ( thetaphiphi != thetaphiphi ) then
                     !  write(*,*) 'thetaphiphi is NaN'
                      ! !print **, omega,phic,sigma,dentst
                      ! !print **, "omega,phic,sigma"
...
!1st partial derivative of potential function wrt efolds
	  real(dl) function V_phi(coff,phi)  
		...
		implicit none
		real(dl) y1,m,phi,H,phi_n,p1,p2,f
		real(dl) coff(25)
		real(dl) cof
		cof=coff(6)
                f=coff(4)
		...
		!V_phi=cof*phi
        V_phi=(-4.d0*cof*phi**3)/f**4


        !(1.632993161855452d0*cof*(-1.d0+  &
        !Exp(0.816496580927726d0*phi)))/Exp(1.632993161855452d0*phi)
       
       ! V_phi=(2*Sqrt(0.66666666666666660)*(-1 + Exp(Sqrt(0.66666666666666660)*phi))*cof)/Exp(2*Sqrt(0.66666666666666660)*phi)
	...
	implicit none
		real(dl)   y2,m,phi,p1,p2,p3,p4,f
		real(dl) coff(25)
		real(dl) cof
		cof=coff(6)
                f=coff(4)
		
		V_phi_phi=(-12.d0*cof*phi**2)/f**4
                !(-1.3333333333333333d0*cof*(-2.d0+  &
                ! Exp(0.816496580927726d0*phi)))/Exp(1.632993161855452d0*phi)
		!V_phi_phi=cof
	return
	end function
...
!	        V_phi_phi_phi=0.0
        ...
        V_phi_phi_phi=(-24.d0*cof*phi)/f**4
        !(1.0886621079036347d0*cof*(-4.d0+  &
        !Exp(0.816496580927726d0*phi)))/Exp(1.632993161855452d0*phi)
	return
	end function
	...
	return
	    ...
	    subroutine write_info(x0,coff,info,filename,jin)
!use mod1
implicit none
		...
		write(1555,*)"##############################Set No:",jin," #############################################"
		write(1555,*)"phi_ini=coff(15)=",coff(15),"hilltop-term=coff(4)",coff(4) 
        write(1555,*) "efold=coff(9)=",coff(9),"x0=",x0 
		write(1555,*) "1st nonminimal coupling term=coff(16)=",coff(16)
        write(1555,*) "power of 1st nonminimal coupling term=coff(25)=",coff(25)
        write(1555,*) "2nd nonminimal coupling term coefficient ratio=coff(23)=",coff(23)
        write(1555,*) "denominator power p:coff(20)= ",coff(20),"overall denominator power q:coff(21)= ",coff(21)
        write(1555,*) "numerator power m:coff(22)= ",coff(22), "coeffient in log term s2:coff(19)",coff(19)
		...
		write(1555,*) "info(4)=ns(avarage)=",info(4),"info(5)=ns(avarage)",info(5)
